<x-app-layout>
    ...
    <div x-data="{ showModal: false, message: '' }" x-init="@if (session('flash_message')) message = '{{ session('flash_message') }}';
        showModal = true; @endif" x-cloak x-show="showModal"
            ...
            <form form method="POST" action="{{ route('results.api') }}" class="py-2 gap-3 space-x-4">
                ...
                <div class="flex justify-center gap-2 items-center">
                    ...
                    <div class="flex items-center space-x-2">
                        <div class="relative">
                            <select name="region"
                                class="bg-transparent text-gray-500 font-bold focus:outline-none border border-gray-300 px-4 py-2">
                    ...
                    <div class="flex items-center space-x-2 border-gray-300 px-4 py-2 rounded-md">
                        <input name="gameId" type="text" placeholder="ゲーム名 + #"
                            class="bg-transparent text-gray-500 font-bold focus:outline-none">
                    </div>
                    <button type="submit"
                        class="ml-4 px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-700">ユーザー検索</button>
                </div>
            </form>
        </div>
    </div>
                                ...
                                <div class="flex items-center justify-between">
                                    ...
                                    <div x-data="{ open: false }" class="relative text-sm font-normal">
                                        <button @click="open = true"
                                            class="ml-4 px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-700 flex items-center">
                                            <span>ブックマークに追加</span>
                                            <svg class="w-4 h-4 ml-2 duration-200 ease-out" viewBox="0 0 24 24"
                                        ...
                                        <div x-cloak x-show="open"
                                            ...
                                            <div x-on:click.away="open = false"
                                                class="bg-white dark:bg-gray-800 p-6 rounded-lg w-1/3">
                                                <div class="flex justify-between">
                                                    <h2 class="text-lg font-bold mb-4">Bookmarks</h2>
                                                    <div x-on:click="open = false" class="text-2xl cursor-pointer">×
                                                    </div>
                                                </div>
                                                <div class="items-center mb-4">
                                                    @foreach ($bookmarks as $bookmark)
                                                        <form
                                                            action="{{ $result->bookmark_contents->contains($bookmark->id) ? route('bookmarkcontents.destroy', $bookmark) : route('bookmarkcontents.store', $bookmark) }}"
                                                            method="POST">
                                                            @csrf
                                                            @if ($result->bookmark_contents->contains($bookmark->id))
                                                                ...
                                                                <input id="{{ $bookmark->id }}"
                                                                    onchange="submit(this.form)" checked type="checkbox"
                                                                    class="w-4 h-4 bg-gray-100 border-gray-300 rounded text-neutral-900 focus:ring-neutral-900">
                                                            @else
                                                                <input type="hidden" name="result_id"
                                                                    value="{{ $result->id }}">
                                                                <input id="{{ $bookmark->id }}"
                                                                    onchange="submit(this.form)" type="checkbox"
                                                                    class="w-4 h-4 bg-gray-100 border-gray-300 rounded text-neutral-900 focus:ring-neutral-900">
                                                            @endif
                                                            <label for="{{ $bookmark->id }}"
                                                                class="ml-2 text-sm font-medium text-gray-900">{{ $bookmark->name }}</label>
                                                            <br>
                                                        </form>
                                                    @endforeach
                                                            ...
                                                            <form method="POST"
                                                                ...
                                                                <div class="flex items-center">
                                                                    ...
                                                                    <button type="submit" @click="show = false"
                                                                        class="ml-4 px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-700">作成</button>
                                                                </div>
                                                            </form>
                                                        </div>
                                                    </div>
